...
public class FrmCadastroMidias extends JFrame {
	...
	private JTextField txt_valor_venda;
	private JLabel lblValorDeComissaor;
	private JTextField txt_valor_comissao_contador;
	...
	public FrmCadastroMidias() {
		...
		txt_valor_venda = new JTextField();
		txt_valor_comissao_contador = new JTextField();
		txt_valor_comissao_contador.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				...
				String caracteres="0987654321.";
			       ...
			       if (txt_valor_comissao_contador.getText().length()>caracteres_maximo) {
			    	   e.consume();
			    	   JOptionPane.showMessageDialog(null, "Maximo de caracteres permitido");
					
				}
			       
//			       if (txt_valor_comissao_contador.getText().contains(".")) {
//			    	   
//			    	   
...
//				}
		...
		lblQuantidadeInicial.setBounds(10, 117, 136, 14);
		...
		txt_quantidade.setBounds(126, 114, 59, 20);
		...
		btnGravar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (midiaEditar == null) {
					...
					if (txt_valor_venda.getText().equalsIgnoreCase("")) {
						txt_valor_venda.setBackground(Color.yellow);
						return;
					}else {
						txt_valor_venda.setBackground(Color.white);
					}
					
					double venda = Double.parseDouble(txt_valor_venda.getText());
					
					
					if (txt_valor_comissao_contador.getText().equalsIgnoreCase("")) {
						txt_valor_comissao_contador.setBackground(Color.yellow);
						return;
					}else {
						txt_valor_comissao_contador.setBackground(Color.white);
					}
					
					double comissao_contador = Double.parseDouble(txt_valor_comissao_contador.getText());
					
					
					
					
					try {
						MySql.getInsereMidia(descricao, quantidade, custo, venda, comissao_contador);
					...
					int idAlunoEditar = midiaEditar.getId();
					...
					midiaEditar.setPreco_venda(venda);
					midiaEditar.setValor_comissao(comissao_contador);
		...
		JLabel label_valor_venda = new JLabel("Valor de venda R$");
		label_valor_venda.setBounds(224, 84, 121, 14);
		contentPane.add(label_valor_venda);
		
		
		txt_valor_venda.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent arg0) {
				...
				String caracteres="0987654321.";
			       ...
			       int caracteres_maximo=5;
				if (txt_valor_venda.getText().length()>caracteres_maximo) {
			    	   arg0.consume();
			    	   JOptionPane.showMessageDialog(null, "Maximo de caracteres permitido");
		...
		txt_valor_venda.setBounds(348, 81, 86, 20);
		contentPane.add(txt_valor_venda);
		txt_valor_venda.setColumns(10);
		
		lblValorDeComissaor = new JLabel("Valor de comissao R$");
		lblValorDeComissaor.setBounds(457, 86, 136, 14);
		contentPane.add(lblValorDeComissaor);
		
	
		txt_valor_comissao_contador.setColumns(10);
		txt_valor_comissao_contador.setBounds(603, 83, 86, 20);
		contentPane.add(txt_valor_comissao_contador);
		...
		tabela_midias.setModel(new DefaultTableModel(
			...
			new String[] {
				"ID", "DESCRIÇÃO", "QTD", "CUSTO", "VENDA", "VL COMISSAO"
			}
		));
		...
		try {
			...
			while (resultSet.next()) {
				...
				Object[] linha1 = {m.getId(),m.getDescricao(),m.getQuantidade(),"R$ "+m.getCusto(),"R$ "+m.getPreco_venda(),"R$ "+m.getValor_comissao(),};
		...
		tabela_midias.getColumnModel().getColumn(1).setMaxWidth(268);
		tabela_midias.getColumnModel().getColumn(2).setMaxWidth(50);
		tabela_midias.getColumnModel().getColumn(3).setMaxWidth(110);
		tabela_midias.getColumnModel().getColumn(4).setMaxWidth(110);
		tabela_midias.getColumnModel().getColumn(5).setMaxWidth(110);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (opc == 0) { // opcao 0 = sim
						...
						txt_valor_comissao_contador.setText(m.getValor_comissao()+"");
						txt_valor_venda.setText(m.getPreco_venda()+"");
...
DefaultTableModel model = (DefaultTableModel)tabela_midias.getModel();
		...
		lblObsNosCampos = new JLabel("Obs: Nos campos de valores R$ n\u00E3o use mais de um \".\" ponto para evitar erros.");
		...
		contentPane.setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[]{txt_descricao, txt_custo, txt_quantidade, btnGravar, label_descricao, lblCustoR, lblQuantidadeInicial}));
	}
